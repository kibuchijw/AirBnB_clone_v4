...
$(document).ready(function () {
  ...
  function fetchAndDisplayReviews () {
    ...
    const reviewsToggle = $('#showReviews');

    // Check if the text is "hide"
    if (reviewsToggle.text() === 'hide') {
      // If text is "hide", remove all Review elements from the DOM
      reviewsSection.find('.review').remove();
      // Change the text to "show"
      reviewsToggle.text('show');
    } else {
      // If text is not "hide", fetch and display reviews
      $.get('http://' + window.location.hostname + ':5001/api/v1/places_reviews/', function (data) {
        // Clear existing reviews in the reviewsSection
        reviewsSection.find('.review').remove();

        // Loop through the result and create review elements
        for (const review of data) {
          const reviewElement = $('<li class="review">');
          reviewElement.text(review.text);
          // Append the review to the reviewsSection
          reviewsSection.find('ul').append(reviewElement);
        }
